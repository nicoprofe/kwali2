...
export default function ProfileApproval() {
    ...
    const [ isPreview1Accepted, setIsPreview1Accepted ] = useState(false)
    const [ isPreview2Accepted, setIsPreview2Accepted ] = useState(false)
                            ...
                            <>
                                <div className='flex items-center justify-around mt-3 ' key={order.id}>
                                    ...
                                    <div className='flex flex-col items-center justify-center space-y-3'>
                                        ...
                                        <button 
                                        disabled={isPreview1Accepted === true}
                                        onClick={() => {aproveFirst.openModal()}}
                                        className={`bg-${isPreview1Accepted ? 'green-500 text-white' : 'secondary-blueLight hover:bg-sky-300 active:bg-sky-400' } 
                                        text-xs active:text-white px-4 py-1 transition duration-200 ease-in-out rounded`}>
                                        {isPreview1Accepted === true ? 'Prueba #1 aprobada' : 'Aprobar prueba #1' }</button>
                                        
                                        
                                        <button
                                        disabled={isPreview2Accepted === true}
                                        onClick={aproveSecond.openModal} 
                                        className={`bg-${isPreview2Accepted ? 'green-500 text-white' : 'secondary-blueLight hover:bg-sky-300 active:bg-sky-400'} 
                                        text-xs active:text-white px-4 py-1 transition duration-200 ease-in-out rounded`}>
                                        {isPreview1Accepted === true ? 'Prueba #1 aprobada' : 'Aprobar prueba #1' }</button>
